...
`Module` 类是 `torch.nn` 模块里提供的一个模型构造类，是所有神经网络模块的基类，我们继承它来定义我们想要的模型。`Module`可以以树形结构包含其它的`Module`,并且其中包含网络各层的定义及`forward`方法。具体来说，一个nn.Module可以被设计为另一个nn.Module的成员变量，从而形成了模块的嵌套关系。这样的嵌套可以在一个模块内部定义，并且可以无限地进行层级嵌套，从而形成树形结构。例如，假设我们有一个神经网络模型，它由多个子模块组成，每个子模块负责不同的功能。我们可以定义一个主模块作为顶层模块，并将各个子模块作为主模块的成员变量。这样，主模块就包含了这些子模块，并形成了一个树形结构。

换句话说，我们在定义自己所需的神经网络时，一定要继承`Module`类，因为`Module`类中包含了很多基本的和常用的方法。下面继承 Module 类构造多层感知机。这里定义的 MLP 类重载了 `Module` 类的 `__init__` 函数和 `forward` 函数。它们分别用于创建模型参数和定义前向计算（正向传播）。下面的 MLP 类定义了一个具有两个隐藏层的多层感知机。
...
from torchsummary import summary##引入torchsummary查看网络形状
...
net = Net()
print(net)
summary(net,(1,32,32))##输入一个单通道，32×32的图片，查看网络结构
```
运行结果如下所示，展示了每一层的信息和形状：
```python
Net(
...
```
## 2.3.4 模型结构的查看
很多时候，我们会使用到别人的深度学习模型。但是，我们以使用模型的骨架为主，至于模型的参数，则需要我们根据自己的数据集进行训练。另一方面，了解了模型的骨架，也有助于我们更方便地进行debug。那么，我们有时候需要查看模型参数在模型中的作用，或者需要调整模型的骨架，或者进行一些有关模型结构的debug，就首先需要学会如何查看模型的结构。

### 打印模型结构
查看模型最简单的一种方法，就是直接打印，以ResNet18为例进行说明。
```py
...
```
我们可以发现单纯的`print(model)`，只能得出基础构件的信息，既不能显示出每一层的shape，也不能显示对应参数量的大小，为了解决这些问题，我们就需要使用`torchinfo`。
...
```
- torchinfo的使用
torchinfo的使用也是十分简单，我们只需要使用torchinfo.summary()就行了，必需的参数分别是model，input_size[batch_size,channel,h,w]，更多参数可以参考[documentation](https://github.com/TylerYep/torchinfo#documentation)，下面让我们一起通过一个实例进行学习。
...
**注意：**
你使用的是colab或者jupyter notebook时，想要实现该方法，summary()一定是该单元（即notebook中的cell）的返回值，否则我们就需要使用print(summary(...))来可视化。


## 参考资料
1. [可视化网络的结构](https://github.com/datawhalechina/thorough-pytorch/blob/main/docs/%E7%AC%AC%E4%B8%83%E7%AB%A0/7.1%20%E5%8F%AF%E8%A7%86%E5%8C%96%E7%BD%91%E7%BB%9C%E7%BB%93%E6%9E%84.html)
2. ChatGPT
