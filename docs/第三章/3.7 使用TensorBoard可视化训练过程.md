...
![tb_model](./figures/tensorboard_resnet.png)

## 3.7.5 TensorBoard图像可视化
...
![tb_images](./figures/tensorboard_image1.png)

![tb_image_gird](./figures/tensorboard_image3.png)

![tb_image](./figures/tensorboard_image2.png)
...
```
此时开始训练后，可以在tensorboard中看到损失函数的变化趋势。其中深色为平滑值，浅色为真实值。
![](./figures/tb_loss.png)
![](./figures/tb_loss1.png)
注意：可以在右上角的设置中打开自动刷新的设置

![](./figures/set.png)

## 3.7.7 TensorBoard参数分布可视化
...
对于TensorBoard来说，它的功能是很强大的，可以记录的东西不只限于本节所介绍的范围。这个功能为模型训练提供了全面的可视化和监控。

其主要的实现方案是构建一个`SummaryWriter`，然后通过`add_XXX()`函数来实现。
